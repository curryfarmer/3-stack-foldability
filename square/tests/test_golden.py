...
The files in square/tests/golden/ (written by square/tests/gen_golden.py) are the fidelity lock: they were
...
def _params(prefix: str) -> list[Any]:
    ...
    If square/tests/golden/ is missing or has no matching files, returns a single param carrying None so the
...
def _skip_if_empty(payload: dict[str, Any] | None) -> None:
    ...
    if payload is None:
        pytest.skip("no matching golden files in square/tests/golden/ — run square/tests/gen_golden.py")

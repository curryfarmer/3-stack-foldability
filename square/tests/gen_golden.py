...
diffs new engine output against these files. Output -> square/tests/golden/*.json + INDEX.json.

Usage:
  python square/tests/gen_golden.py            # generate everything, print timing
  python square/tests/gen_golden.py probe      # just probe physical deciders, write nothing

This is a SQUARE-side script: it bootstraps square/ only and must never co-import triangle/ (both
ship a bare-named `lattice`; see square/tests/conftest.py for the full rationale).
"""
...
PKG = os.path.dirname(HERE)                     # square/
ROOT = os.path.dirname(PKG)                     # repo root
sys.path.insert(0, PKG)
import _bootstrap  # noqa: E402,F401  (square/ + engine/ + twist/ + render/)
sys.path.insert(0, HERE)

import enginelib as EL  # noqa: E402

GOLDEN = os.path.join(HERE, "golden")
# Physical-fold labels, TRACKED alongside the corpus. This used to read results/twoplus1_labels.json,
# which does not exist -- so probe_deciders() raised FileNotFoundError in EVERY mode, aborting the
# script after the goldens had already been written (nonzero exit, artifacts on disk).
...
# 6x4/6x5/6x6 nc=True are added because the 2026-06 fold campaign's ground truth is OFF-CORNER (the
# foldable 2+1 survivors live off-corner), so the corner vet sets do not contain those decider hashes.
# 6x6 nc=True is the slowest vet target (closing_candidates >4 min) — it is the oracle for the 40
# off-corner 6x6 FOLD deciders ingested in the 2026-06 campaign; regenerate with `gen_golden.py vetonly`.
...
def probe_deciders() -> list[dict]:
    ...
    the engine's foldable tag to the recorded physics.

    Reporting only -- it writes nothing and never decides the exit code, so a missing labels file
    degrades to a warning rather than destroying an otherwise-complete generation run.
    I/O: () -> [{grid,id,physical,...}] ([] when the labels file is absent)."""
    if not os.path.exists(LABELS):
        print(f"  (no labels at {os.path.relpath(LABELS, ROOT)} — skipping decider probe)")
        return []

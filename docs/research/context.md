...
**⚠ CORRECTION (2026-06-09) — the redundancy conclusion above is RETRACTED.** It was an artifact
of the *mis-ported* reflection gate. The old `reflection_check` seeded an arbitrary `{edge:'T',
sign:1}` at `baseCells[0]` of every chain (NOT the crease the chains share) and compared lossy
`(edge,sign)` LABELS — so for 2+1 it was vacuous (everything passed), which is why it looked
redundant with parity. The 2026-06-08 physical jam of `6x5_1` (which parity+old-reflection both
passed) exposed it. The corrected gate (`fold.reflection_verdict`, faithful to `twostack.py`):
seed the **shared crease** as one world segment, reflect each chain to its far end (equal #
reflections), and require the two images to **coincide as oriented grid segments** (endpoints +
direction), not as labels. This gate is **BINDING, not output-preserving**:
- 2+1: rejects both physical jams (`6x5_1`, `6x7_8`); per-grid PASS = 6x4 1/2, 6x5 3/5, **6x6 0/28
  (all jam)**, 6x7 33/41, 9x4 11/11. (Counts over distinct closing 2+1 candidates.)
- 1+1+1: the same "side-sharing" math applied to each footprint crease rejects **51 of 292**
  previously-accepted solutions.
Caches regenerated with the corrected gate (2026-06-09). The Q4c "no sign condition needed" and
Q4b "(B,+1) coincidence" observations described the OLD label gate and no longer characterise the
accepted set. Cross-checked Python ≡ JS (615 solutions, 0 verdict mismatches).

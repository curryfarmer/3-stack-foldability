...
## 2026-06-09 — fixed orientation-aware vector reflection (2+1 + 1+1+1); reflection gate is BINDING

**What was wrong.** The engine's `reflection_check` was a mis-port of the 2-stack reference
(`twostack.py`): it seeded an arbitrary director `{edge:'T', sign:1}` at `baseCells[0]` of *every*
chain — not the crease the chains share — and compared lossy `(edge,sign)` LABELS. For 2+1 it was
vacuous (everything passed), which is why Q4 (2026-05-28) concluded it was redundant/output-
preserving. The 2026-06-08 physical JAM of `6x5_1` (parity + old-reflection both passed) exposed
it; john's read: "the 2-chain vector doesn't start on the shared crease with the 1-chain vector."

**The fix (`fold.reflection_verdict`, faithful to the reference).** Seed the **shared crease**
between each adjacent pair of chains as ONE world segment; reflect each side to its far end (equal
# reflections); the pair rejoins iff the two images **coincide as oriented grid segments**
(endpoints + direction), not as labels — because the B edge of cell (a,b) and the T edge of (a,b+1)
are the *same grid line*. 2+1 → one shared crease; 1+1+1 → each footprint crease (the "side-
sharing" generalisation john asked for). Ported to `search.py`, `search.js`, `fold.js`.

**Validation.** Rejects BOTH physical jams: `6x5_1` (2-chain image on grid line y=5, 1-chain on
y=3 → different lines → JAM) and `6x7_8`. Non-degenerate per-grid 2+1 PASS: 6x4 1/2, 6x5 3/5,
**6x6 0/28 (ALL jam)**, 6x7 33/41, 9x4 11/11. 1+1+1 side-sharing rejects **51/292** previously-
accepted. Cross-engine Python ≡ JS: 615 solutions, **0** verdict mismatches.

**Consequences.** (1) Q4 "reflection redundant / output-preserving" RETRACTED — the gate is now
binding. (2) NEW high-info predictions to fold: **all 6x6 2+1 jam** (0 foldable on 6x6), incl. the
old `6x6_7` "both-FOLD" control — if 6x6_7 physically FOLDS, the segment-coincidence predicate is
too strict and must be revisited. (3) Caches regenerated (`--force`): 6x5 5→3, 6x6 30→12, 6x7
41→33, 9x4 26→19, 6x4 3→2. (4) All fold sheets regenerated to draw the shared-crease seed + the
two reflected far-end images + PASS/FAIL verdict (`render_foldpath`, `render_vectors`,
`vet_enumerate`); deciders rendered from `twoplus1_labels.json` (cache-independent).

**CONFIRMED (2026-06-09, later same day).** john physically folded the strongest 6x6 2+1 candidate
(= `6x6_1`: footprint (0,0)+(0,1)+(1,0), closes + parity-OK) → **JAMS**, exactly as the corrected
gate predicts (images A→(3,3)-(3,4) −y vs B→(1,3)-(1,4) −y, x=3≠x=1). First FOLD-side test of the
segment-coincidence predicate on 6x6 → **predicate holds, not too strict**; all-6x6-2+1-jam stands.

**BIG FINDING (2026-06-09): the corner-only restriction (`allowNonCorner:False`) was hiding the
bulk of foldability — and ALL foldable 6x6 2+1.** Re-ran with footprints allowed anywhere, split by
decomp/anchor (predicted-FOLD = parity+reflection+twist all pass):

| grid | decomp | corner-only FOLD | non-corner total FOLD (corner/edge/interior) |
|---|---|---|---|
| 6x4 | 2+1   | 1  | 8  (1 / 5 / 2) |
| 6x4 | 1+1+1 | 1  | 5  (1 / 4 / 0) |
| 6x5 | 2+1   | 3  | 9  (3 / 2 / 4) |
| 6x5 | 1+1+1 | 0  | 4  (0 / 2 / 2) |
| 6x6 | 2+1   | **0/26** | **40 (0 / 18 / 22)** |
| 6x6 | 1+1+1 | 12 | 67 (12 / 51 / 4) |

So "6x6 2+1 doesn't fold" is a **corner-anchoring artifact, not a 2+1 obstruction**: 0 of 26
corner-anchored 6x6 2+1 fold (one physically confirmed jam, `6x6_1`), but **40 non-corner 6x6 2+1
are predicted to fold** (none at a corner — all edge/interior). All non-corner predicted-FOLDs are
UNTESTED on paper. Rendered the full predicted-FOLD set as path sheets `report/foldsheets/
<grid>_fold_<NN>_<decomp>_<anchor>.png` (6x4 ×13, 6x5 ×13, 6x6 ×1 valid 2+1, edge-anchored
(3,0)+(3,1)+(4,0)); pruned all other foldsheets/PDFs.

**CONFIRMED FOLD (2026-06-09): `6x6_fold_01` folds flat.** john physically folded the edge-anchored
6x6 2+1 (L footprint (3,0)+(3,1)+(4,0): rigid vertical domino A=(3,0)+(3,1) + single cell B=(4,0),
one shared crease at x=4). Same rigid-domino 2+1 MODEL as the jamming `6x6_1` — only the footprint
moved off the corner. Reflection PASS: both far-end images land on the SAME oriented segment
(4,3)-(4,4) −y (vs `6x6_1`'s split x=1/x=3). **First FOLD-side confirmation of the corrected gate**,
and proof that non-corner 6x6 2+1 are real, not artifacts. The gate now has BOTH-sided validation on
6x6 2+1: corner jams (`6x6_1`), edge folds (`6x6_fold_01`).

**Next.** Fold more non-corner predicted-FOLDs — the 6x5 interior 2+1s, and a 6x6 interior 2+1. Re-run the whole vet/cache pipeline with
`allowNonCorner:True` (it currently ships `False`, so caches understate foldability). Note:
`6x4`/`6x5`/`6x6` had stale older-hash 3-stack cache duplicates; reconciled to one file each.

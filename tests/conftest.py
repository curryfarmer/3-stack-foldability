...
Puts the Python engine packages on sys.path (py/, py/tri/, explainer/) so test
modules can `import search`, `import fold`, `import twostack`, etc. exactly as the
engine modules import one another. Also exposes shared directory fixtures.
"""
import os
import sys
...
for _p in (HERE, PY, os.path.join(PY, "tri"), os.path.join(ROOT, "explainer")):
